...
import { BsCart3, BsHeart, BsPerson, BsSearch, BsCamera, BsList, BsX } from 'react-icons/bs';
  ...
  return (
    <>
      <nav className="bg-[#0a0f1d] border-b border-gray-800 fixed w-full top-0 left-0 z-50 h-20 flex items-center shadow-lg">
        ...
        {/* Mobile Menu Overlay */}
        <div className={`lg:hidden fixed inset-0 z-40 bg-black/60 transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`} onClick={() => setIsMobileMenuOpen(false)}>
          <div
            className={`absolute left-0 top-0 h-full w-4/5 bg-[#0a0f1d] p-8 transition-transform duration-300 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}
            ...
            <div className="flex flex-col space-y-6 mt-10">
              {[
                ...
                <Link key={item.name} href={item.href} onClick={() => setIsMobileMenuOpen(false)}>
                  <span className="text-lg font-black text-white tracking-widest border-b border-white/10 pb-2 flex items-center">
                    {item.name}
                  </span>
                </Link>
              ))}
